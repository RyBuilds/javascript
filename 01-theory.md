...
### Simple:
JavaScript is a high-level, object-orientated, multi-paradigm programming language.
...
## How do JavaScript Engines Compile Source Code?
...
Modern JavaScript have clever optimisation strategies; initially they create a very unoptimised version of machine code, just so it can start executing as fast as possible. Then in the background, this code is being optimised and recompiled during the already running program execution, which can be done multiple times. After each optimisation, the unoptimised code is simply swapped for the new and more optimised code without ever stopping execution. This process is what makes modern engines so fast. All this parsing, compilation and optimisation happens in some special threads inside the engine which we cannot access from our code, so completely seperate to the main thread running in the callstack and executing our own code.

## JavaScript Runtime in the Browser

A JavaScript runtime is a big container including all the things we need in order to use JavaScript.

<details>
<summary>JavaScript Engine</summary>

The heart of any JavaScript runtime is always the JavaScript engine - without an engine there's no runtime and there's no JavaScript at all - however the engine alone is not enough; in order to work properly we also need access to the `Web APIs`.
...
Web APIs are functionalities provided to the engine that are not part of the JavaScript languge; everything related the DOM, Timers, Fetch API etc. JavaScript gets access to these APIs through the global window object.

</details>

<details>
<summary>Callback Queue</summary>

Typical JavaScript runtimes also include a `Callback Queue`, a data structure which includes all the callback functions that are ready to be executed. Event handler functions are also called `callback functions`, and so when you attach event handler functions to DOM elements, the callback function will be called once the event happens.

For example, an event handler reacting to a click on a button. The first thing that happens after the click, is that the callback function is put into the callback queue. Then when the callstack is empty, the callback function is passed to the stack so that it can be executed. The `event loop` is responsible for this process; event loops take callback functions from the callback queue and puts them in the callstack so that they can be executed. The event loop is how JavaScripts non-blocking concurrency model is implemented.

</details>

## JavaScript Runtime in Node.js

Runtime is pretty similar, bit since we dont have a browser - we cant have the web APIs since its the browser which provides these. Instead, we have multiple C++ bindings and a thread pool.
